...
public class ConnectionHandler {
    ...
    public void registerCommands() {
        ...
        //Passive approach Passive approach，Passive approach
        //Proactive approach Proactive approach Proactive approach
        con.registerCommand("PASV", "PASV", this::pasv); // Passive Mode
        //Belowport Below pasv 2021Below7Below9Below09:26:59 wddBelow
        con.registerCommand("PORT", "PORT <address>", this::pasv); // Active Mode
    ...
    private void pasv() throws IOException {
        ...
        int pasvPort = random.nextInt(100) + 50200;

        Socket socket = con.getSocket();
        InetAddress inetAddress = socket.getInetAddress();

        Utils.portCheck(pasvPort,inetAddress.getHostAddress());

        passiveServer = Utils.createServer(pasvPort, 5, server.getAddress(), server.getSSLContext(), secureData);
        passive = true;

        Utils.serMap.put(pasvPort,passiveServer);

        String host = passiveServer.getInetAddress().getHostAddress();
        int port = passiveServer.getLocalPort();
      //  log.info("pasv before -> " + host);
        if (host.equals("0.0.0.0") || host.equals("127.0.0.1") || host.equals("localhost")) {
            host = IPUtil.getIpAddr(server.getIP_HOST());
            if (null == host) {
                host = InetAddress.getLocalHost().getHostAddress();
            }
        }
     //   log.info("pasv final -> " + host);
