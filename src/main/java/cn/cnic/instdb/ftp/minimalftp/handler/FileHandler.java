...
package cn.cnic.instdb.ftp.minimalftp.handler;
...
import cn.cnic.instdb.ftp.minimalftp.api.ResponseException;
...
public class FileHandler {
    ...
    private Object getFile(String path) throws IOException {
        if(path.equals("...") || path.equals("..")) {
            ...
            return fs.getRoot();
        } else if(path.contains(fs.getRoot().toString())) {
            //Restricted access situation  Restricted access situation  Restricted access situationrootRestricted access situation Restricted access situation Restricted access situation
    ...
    private void rnto(String path) throws IOException {
        ...
        if(StringUtils.isNotBlank(role) && Constant.GENERAL.equals(role)){
            con.sendResponse(550, "No permission");
            return;
        }
    ...
    private void retr(String path) throws IOException {
        ...
        String result = con.getControl().resourcesDownloadFile(file.getPath(), con.getResourceId(), con.getRealUsers(), inetAddress.getHostAddress());
        if (!"ok".equals(result)) {
            file = (File) getFile(result);
        }

        con.sendResponse(150, "Sending the file stream for " + path + " (" + file.length() + " bytes)");
    ...
    private void mlsd(String[] args) throws IOException {
        ...
        List<ResourceFileTree> list = con.getControl().listFiles(con.getResourceId(), file1.getPath(),con.getRealUsers());

        StringBuilder data = new StringBuilder();
        if (null != list && list.size() > 0) {
            for (ResourceFileTree l :list) {
                data.append(Utils.getFactsNew(fs, l, options));
            }
        }
        con.sendData(data.toString().getBytes("UTF-8"));
